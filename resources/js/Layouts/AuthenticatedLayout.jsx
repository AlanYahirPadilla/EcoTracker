...
import { LayoutDashboard, Recycle, Gift, Trophy, History, Ticket, User, LogOut, Menu, X, ChevronRight, ChevronLeft } from 'lucide-react';
...
export default function Authenticated({ user, header, children }) {
    ...
    const menuItems = [
        ...
        { name: "Registrar Reciclaje", href: route('recycle'), icon: Recycle },
        { name: "Recompensas", href: route('rewards'), icon: Gift },
        { name: "Ranking", href: route('ranking'), icon: Trophy },
        { name: "Historial de Canjes", href: route('rewards.history'), icon: History },
        { name: "Mis Tickets", href: route('tickets'), icon: Ticket },
    ];
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex justify-between h-16">
                            <div className="flex items-center">
                                {/* Eliminado el botón de hamburguesa */}
                        ...
                        <div className="pt-2 pb-3 space-y-1">
                            {menuItems.map((item) => (
                                <ResponsiveNavLink 
                                    key={item.name}
                                    href={item.href} 
                                    active={route().current(item.href.replace(window.location.origin, ''))}
                                >
                                    {item.name}
                                </ResponsiveNavLink>
                            ))}
